...
from storage.schema import SnapshotBundle, DeviceStatus

def _dt_to_str(dt: datetime | None) -> str | None:
    """Преобразует datetime в строку ISO для SQLite."""
...
class Repository:
    ...
    def save_bundle(self, bundle: SnapshotBundle) -> None:
        """
        Сохраняет весь пакет атомарно в одной транзакции.
        """
        ...
        try:
            ...
            # 2. Сохраняем/Обновляем Device (Золотое правило: обновляем только last_seen)
            if bundle.device:
                cursor.execute("""
            ...
            # 7. Сохраняем CollectorLog (если есть)
            if bundle.collector_log:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    bundle.collector_log.id,
                    bundle.collector_log.scan_id,
