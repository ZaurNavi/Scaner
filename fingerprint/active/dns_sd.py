...
class DnsSdCollector(ActiveCollector):
    ...
    def __init__(self, configuration: ConfigurationManager):
        ...
        self.services = ["_airplay._tcp.local", "_googlecast._tcp.local", "_ipp._tcp.local", "_printer._tcp.local", "_raop._tcp.local", "_http._tcp.local"]

    def collect(self, device: Device) -> FingerprintResult:
        start_time = time.time()
        cached = cache_get(device.ip, "dns_sd")
        if cached: return FingerprintResult(**cached, source="dns_sd", elapsed_ms=0.0)
        if not self.is_available(device):
            return FingerprintResult(source="dns_sd", raw_data={"responded": False, "reason": "device_unavailable"}, elapsed_ms=(time.time() - start_time) * 1000)
        
        sd_data = self._query_dns_sd(device.ip)
        elapsed_ms = (time.time() - start_time) * 1000
        result = FingerprintResult(source="dns_sd", raw_data=sd_data or {"responded": False, "reason": "no_services_found"}, elapsed_ms=elapsed_ms, capabilities=["supports_dns_sd"] if sd_data and sd_data.get("services") else [])
    ...
    def _query_dns_sd(self, ip: str) -> dict | None:
        ...
        try:
            ...
            for service in self.services:
                ...
                for part in service.split('.'): query += bytes([len(part)]) + part.encode('utf-8')
    ...
    def scan(self, devices: list[Device], context: dict | None = None, **kwargs) -> dict[str, FingerprintResult]:
        ...
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                except Exception: results[ip] = FingerprintResult(source="dns_sd", elapsed_ms=0.0)
        return results
